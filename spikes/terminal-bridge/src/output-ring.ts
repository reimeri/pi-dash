...
export class OutputRing {
  ...
  push(data: string): SequencedOutput {
    const entry = { seq: ++this.#latestSeq, data, bytes: Buffer.byteLength(data, "utf8") };
  ...
  canReplayFrom(seq: number): boolean {
    return seq >= this.earliestSeq && seq <= this.latestSeq + 1;
  }

  replayFrom(seq: number): readonly SequencedOutput[] {
    if (!this.canReplayFrom(seq)) return [];
